...
void Adafruit_TFTLCD::moveDown(int x1,int x2,int y1,int y2,double speed,uint16_t color)//方块移动
{

  
  }

void Adafruit_TFTLCD::drawString(int16_t x, int16_t y, String s, uint16_t color,
      uint16_t bg, uint8_t size)
    {
        for(int i=0;i<s.length();i++)
        {
          tft.drawChar(x+6*i*size,y,s[i],color,bg,size);
        }
      }