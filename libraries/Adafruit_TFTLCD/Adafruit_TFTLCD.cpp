...
扩展内容
********************************************************/
/*void Adafruit_TFTLCD::moveDown(int16_t x, int16_t y, int16_t w, int16_t h, 
uint16_t c,double sp,uint16_t bgcolor,uint16_t pressColor)//矩形移动
{
  #define TS_MINX 250
  #define TS_MINY 250
  #define TS_MAXX 880
  ...
  while(y<320)
  {
    ...
    pinMode(YP, OUTPUT);
    t=ts.getPoint();
    t.x=map(t.x,TS_MINX,1023,0,320);
    t.y=map(t.y,TS_MINY,1023,0,240);
    if(t.x>y && t.x<y+h  && t.y>x && t.y<x+w)
    {
      fillRect(x,y,w,h,pressColor);
    }
    else
    {
      fillRect(x,y,w,h,c);
